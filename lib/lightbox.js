...
define('lightbox',['jquery','jquery.mousewheel'],function($){
    var $box,$wrap,$img,$mask,imgFocus = false,ratio=false,initRect = null,initIndex = 0;
    /**
     * 重新计算图片大小
     * @param img
     */
    var calImg = function(img){
        ...
        tempImage.onload = function() {
            //获取当前窗口容许图片显示的最大宽高值
            var maxWidth = $(window).width()*80/100,maxHeight = $(window).height()*95/100;
            //若图片大小大于阈值 则调整大小
            var rect = {
            ...
            if($.browser.msie && parseInt($.browser.version,10)<10){
                $wrap.removeClass('shadow').stop().animate({
                    ...
                    //调整左右按钮位置
                    $('.prev',$box).animate({'left':($(window).width()-$wrap.width())/4+"px"});
                    $('.next',$box).animate({'right':($(window).width()-$wrap.width())/4+"px"});
                });
            }else{
                $wrap.css({
                    'margin-top':(-rect.height/2)+"px",
                ...
                //调整左右按钮位置
                $('.prev',$box).css({'left':($(window).width()-rect.width)/4+"px"});
                $('.next',$box).css({'right':($(window).width()-rect.width)/4+"px"});
            }
    ...
    return {
        ...
        lightbox:function(images,i,rect){
            ...
            var html = '<div class="lightbox" onselectstart="return false">' +
        ...
        zoomInImage:function(event,scale){
            ...
            var newLeft  = -((event.pageX- offsetLeft)/(width/2 ))*(width*(scale?(scale-1)/2:.2))+left;
            var newTop   = -((event.pageY- offsetTop )/(height/2))*(height*(scale?(scale-1)/2 :.2))+top;
            $img.animate({
        ...
        zoomOutImage:function(event,scale){
            ...
            var wrapLeft = $wrap.offset().left;
            var wrapTop  = $wrap.offset().top;
            var newLeft  = ((event.pageX- offsetLeft)/(width/2 ))*(width*(scale?(1-scale)/2:.2))+left;
            var newTop   = ((event.pageY- offsetTop )/(height/2))*(height*(scale?(1-scale)/2:.2))+top;
            $img.animate({
